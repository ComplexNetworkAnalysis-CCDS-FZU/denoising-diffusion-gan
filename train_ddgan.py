...
def extract(input, t, shape):
    """提取器
    从提供的 `input` 中 按照行 提取给定的`t` 的位置对应的Tensor
    然后将得到的向量转为对应的Tensor
...
def get_sigma_schedule(args, device):
    ...
    Returns:
        Tuple[Tensor, Tensor, Tensor]: (sigma, alpha, beta)
    """
    n_timestep = args.num_timesteps
...
def q_sample(coeff, x_start, t, *, noise=None):
    ...
    Args:
        ...
        x_start (Tensor): x_0 初始图像
        t (int): 迭代步数t
        noise (Tensor, optional): 参考噪声. Defaults to None.
...
def q_sample_pairs(coeff, x_start, t):
    """
    生成训练用的加噪声后的图对，即得到 $x_t$ 与 $x_{t+1}$ 处的图
    Generate a pair of disturbed images for training

    Args:
        coeff (_type_): _description_
        x_start (Tensor): x_0 原始图像
        t (int): 加噪的步数

    Returns:
        Tuple[Tensor,Tensor]: x_t, x_{t+1}
    """
    noise = torch.randn_like(x_start)
